...
function SecureNative(apiKey, opts) {
  ...
  options.maxEvents = options.maxEvents || 1000;
...
SecureNative.prototype.sendEvents = function () {
  if (this.events.length > 0 && this.sendEnabled) {
    ...
    rp(eventOptions).catch((err) => {
      this.events.unshift(eventOptions);
...
SecureNative.prototype.track = function (req, opts) {
  ...
  const eventOptions = Object.assign({}, this.clientOptions, {
    uri: this.clientOptions.uri + '/track',
    body: event
  });

  if (this.events.length >= this.optionsmaxEvents) {
    this.events.unshift();
  }
...
SecureNative.prototype.verify = function (req, opts) {
  const eventOptions = Object.assign({}, this.clientOptions, {
    uri: this.clientOptions.uri + '/verify',
    body: this.eventManager.build(req, opts)
  });

  return rp(eventOptions);
};

SecureNative.prototype.middleware = Middleware;
